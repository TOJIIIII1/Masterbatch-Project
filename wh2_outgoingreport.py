...
class Wh2OutgoingReport:
    ...
    def fetch_data_from_wh2_outgoing_report(self):
        """Fetch the latest data from Table 1 with date format MM/DD/YYYY."""
        try:
            self.connect_db()  # Ensure connection is open before executing the query
            query = """
                SELECT wh2_outgoing_report.reference_no, 
                       TO_CHAR(wh2_outgoing_report.date_outgoing, 'MM/DD/YYYY') AS date_outgoing, 
                       wh2_material_codes.material_code, 
                       wh2_outgoing_report.quantity, 
                       wh2_outgoing_report.area_location 
                FROM wh2_outgoing_report
                INNER JOIN wh2_material_codes
                    ON wh2_outgoing_report.material_code = wh2_material_codes.mid
                WHERE wh2_outgoing_report.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 2: Outgoing Report", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh2_outgoing_report, search_entry.get()))
        ...
        column_names_wh2_outgoing_report = [
            "Reference No.", "Date Outgoing", "Material Code", "Quantity", "Area Location"
        ]
        wh2_outgoing_report = ttk.Treeview(
            table_frame,
            columns=column_names_wh2_outgoing_report,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh2_outgoing_report.yview)

        for col_name in column_names_wh2_outgoing_report:
            wh2_outgoing_report.heading(col_name, text=col_name)
            wh2_outgoing_report.column(col_name, width=150, anchor="center")
        wh2_outgoing_report.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh2_outgoing_report = self.fetch_data_from_wh2_outgoing_report()
        self.update_treeview(wh2_outgoing_report, data_wh2_outgoing_report,
                             column_names_wh2_outgoing_report)
        ...
        wh2_outgoing_report_entries = []
        wh2_material_codes = self.fetch_wh2_material_codes()

        for i, label_text in enumerate(column_names_wh2_outgoing_report):
            ...
            if label_text == "Date Outgoing":  # Correct label for the date field
                ...
                wh2_outgoing_report_entries.append(date_entry)
                ...
                wh2_outgoing_report_entries.append(combobox)
            else:
                entry = ttk.Entry(entry_frame, width=15)
                entry.grid(row=1, column=i, padx=10, pady=5)
                wh2_outgoing_report_entries.append(entry)

        self.wh2_outgoing_report_entries = wh2_outgoing_report_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh2_outgoing_report(wh2_outgoing_report),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh2_outgoing_report(wh2_outgoing_report),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh2_outgoing_report(wh2_outgoing_report),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh2_outgoing_report(wh2_outgoing_report, clear_ui_only=True),
            width=10
        )
    ...
    def add_row_wh2_outgoing_report(self, table):
        try:
            ...
            reference_no = self.wh2_outgoing_report_entries[0].get()
            date_outgoing = self.wh2_outgoing_report_entries[1].get()
            material_code = self.wh2_outgoing_report_entries[2].get()
            quantity = self.wh2_outgoing_report_entries[3].get()
            area_location = self.wh2_outgoing_report_entries[4].get()
    ...
    def update_row_wh2_outgoing_report(self, table):
        try:
            ...
            self.cursor.execute("SELECT id FROM wh2_outgoing_report WHERE reference_no = %s", (reference_no,))
            ...
            area_location = self.wh2_outgoing_report_entries[4].get().strip()
            ...
            updated_data = self.fetch_data_from_wh2_outgoing_report()
    ...
    def delete_row_wh2_outgoing_report(self, table):
        ...
        try:
            ...
            query = "SELECT id FROM wh2_outgoing_report WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh2_outgoing_report WHERE id = %s;"
    ...
    def clear_row_wh2_outgoing_report(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh2_outgoing_report SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                ...
                wor.quantity, 
                wor.area_location 
            FROM wh2_outgoing_report wor
            JOIN wh2_material_codes mc ON wor.material_code = mc.mid  -- Updated JOIN condition
