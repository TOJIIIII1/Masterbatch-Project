...
class Wh1TransferForm:
    ...
    def fetch_data_from_wh1_transfer_form(self):
        ...
        try:
            ...
            query = """
                ...
                WHERE wh1_transfer_form.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 1: Transfer Form", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh1_transfer_form, search_entry.get()))
        ...
        column_names_wh1_transfer_form = [
            "Reference No.", "Date Received", "Material Code", "Quantity", "Area To", "Status"
        ]

        wh1_transfer_form = ttk.Treeview(
            table_frame,
            columns=column_names_wh1_transfer_form,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh1_transfer_form.yview)

        for col_name in column_names_wh1_transfer_form:
            wh1_transfer_form.heading(col_name, text=col_name)
            wh1_transfer_form.column(col_name, width=150, anchor="center")
        wh1_transfer_form.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh1_transfer_form = self.fetch_data_from_wh1_transfer_form()
        self.update_treeview(wh1_transfer_form, data_wh1_transfer_form,
                             column_names_wh1_transfer_form)
        ...
        wh1_transfer_form_entries = []
        material_codes = self.fetch_material_codes()

        for i, label_text in enumerate(column_names_wh1_transfer_form):
            ...
            if label_text == "Date Received":
                ...
                wh1_transfer_form_entries.append(entry)

        # Additional Dropdown (Not in Treeview)
        additional_label = ttk.Label(entry_frame, text="New Status", font=("Arial", 12))
        additional_label.grid(row=0, column=len(column_names_wh1_transfer_form), padx=10, pady=5)
        ...
        additional_combobox.grid(row=1, column=len(column_names_wh1_transfer_form), padx=10, pady=5)

        # Store in a class attribute if needed
        self.additional_combobox = additional_combobox

        self.wh1_transfer_form_entries = wh1_transfer_form_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh1_transfer_form(wh1_transfer_form),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh1_transfer_form(wh1_transfer_form),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh1_transfer_form(wh1_transfer_form),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh1_transfer_form(wh1_transfer_form, clear_ui_only=True),
            width=10
        )
    ...
    def add_row_wh1_transfer_form(self, table):
        try:
            ...
            reference_no = self.wh1_transfer_form_entries[0].get()
            date = self.wh1_transfer_form_entries[1].get()
            material_code = self.wh1_transfer_form_entries[2].get()
            ...
            data_wh1_transfer_form = self.fetch_data_from_wh1_transfer_form()
            self.update_treeview(table, data_wh1_transfer_form,
    ...
    def update_row_wh1_transfer_form(self, table):
        try:
            self.connect_db()  # Ensure the connection is open
            ...
            # Retrieve values from entry fields
            date = self.wh1_transfer_form_entries[1].get().strip()
            material_code = self.wh1_transfer_form_entries[2].get().strip()
            quantity = self.wh1_transfer_form_entries[3].get().strip()
            area_to = self.wh1_transfer_form_entries[4].get().strip()
            ...
            query = "UPDATE wh1_transfer_form SET "
            values = []
    ...
    def delete_row_wh1_transfer_form(self, table):
        ...
        try:
            ...
            query = "SELECT id FROM wh1_transfer_form WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh1_transfer_form WHERE id = %s;"
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                ...
                wtf.quantity, 
                wtf.area_to
            FROM wh1_transfer_form wtf
            JOIN material_codes mc ON wtf.material_code = mc.mid  -- Ensure 'mid' is the correct key
    ...
    def clear_row_wh1_transfer_form(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh1_transfer_form SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
