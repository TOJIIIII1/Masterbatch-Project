...
class Wh1OutgoingReport:
    ...
    def fetch_data_from_wh1_outgoing_report(self):
        ...
        try:
            ...
            query = """
                ...
                WHERE wh1_outgoing_report.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 1: Outgoing Report", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh1_outgoing_report, search_entry.get()))
        ...
        column_names_wh1_outgoing_report = [
            "Reference No.", "Date Outgoing", "Material Code", "Quantity", "Area Location"
        ]
        wh1_outgoing_report = ttk.Treeview(
            table_frame,
            columns=column_names_wh1_outgoing_report,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh1_outgoing_report.yview)

        for col_name in column_names_wh1_outgoing_report:
            wh1_outgoing_report.heading(col_name, text=col_name)
            wh1_outgoing_report.column(col_name, width=150, anchor="center")
        wh1_outgoing_report.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh1_outgoing_report = self.fetch_data_from_wh1_outgoing_report()
        self.update_treeview(wh1_outgoing_report, data_wh1_outgoing_report,
                             column_names_wh1_outgoing_report)
        ...
        for i, label_text in enumerate(column_names_wh1_outgoing_report):
            ...
            if label_text == "Date Outgoing":  # Correct label for the date field
                ...
                wh1_outgoing_report_entries.append(date_entry)
                ...
                wh1_outgoing_report_entries.append(combobox)
            else:
                entry = ttk.Entry(entry_frame, width=15)
                entry.grid(row=1, column=i, padx=10, pady=5)
                wh1_outgoing_report_entries.append(entry)

        self.wh1_outgoing_report_entries = wh1_outgoing_report_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh1_outgoing_report(wh1_outgoing_report),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh1_outgoing_report(wh1_outgoing_report),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh1_outgoing_report(wh1_outgoing_report),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh1_outgoing_report(wh1_outgoing_report, clear_ui_only=True),
            width=10
        )
    ...
    def add_row_wh1_outgoing_report(self, table):
        try:
            ...
            reference_no = self.wh1_outgoing_report_entries[0].get()
            ...
            messagebox.showerror("Error", f"Error while adding row to Warehouse 1: Outgoing Report {e}")
    ...
    def delete_row_wh1_outgoing_report(self, table):
        ...
        try:
            ...
            query = "SELECT id FROM wh1_outgoing_report WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh1_outgoing_report WHERE id = %s;"
    ...
    def clear_row_wh1_outgoing_report(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh1_outgoing_report SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                ...
                wor.quantity, 
                wor.area_location 
            FROM wh1_outgoing_report wor
            JOIN material_codes mc ON wor.material_code = mc.mid  -- Updated JOIN condition
