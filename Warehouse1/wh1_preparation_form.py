...
class Wh1PreparationForm:
    ...
    def fetch_data_from_wh1_preparation_form(self):
        """Fetch the latest data from Table 1 with date format MM/DD/YYYY."""
        try:
            self.connect_db()  # Ensure connection is open before executing the query
            query = """
                SELECT wh1_preparation_form.reference_no, 
                       TO_CHAR(wh1_preparation_form.date, 'MM/DD/YYYY') AS date, 
                       material_codes.material_code, 
                       wh1_preparation_form.quantity_prepared, 
                       wh1_preparation_form.quantity_return,  
                       wh1_preparation_form.area_location,
                       wh1_preparation_form.status
                FROM wh1_preparation_form
                INNER JOIN material_codes
                    ON wh1_preparation_form.material_code = material_codes.mid
                WHERE wh1_preparation_form.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 1: Preparation Form", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh1_preparation_form, search_entry.get()))
        ...
        column_names_wh1_preparation_form = [
            "Reference No.", "Date Received", "Material Code", "Quantity Prepared", "Quantity Return", "Area Location", "Status"
        ]
        wh1_preparation_form = ttk.Treeview(
            table_frame,
            columns=column_names_wh1_preparation_form,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh1_preparation_form.yview)

        for col_name in column_names_wh1_preparation_form:
            wh1_preparation_form.heading(col_name, text=col_name)
            wh1_preparation_form.column(col_name, width=150, anchor="center")
        wh1_preparation_form.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh1_preparation_form = self.fetch_data_from_wh1_preparation_form()
        self.update_treeview(wh1_preparation_form, data_wh1_preparation_form,
                             column_names_wh1_preparation_form)
        ...
        wh1_preparation_form_entries = []
        material_codes = self.fetch_material_codes()

        for i, label_text in enumerate(column_names_wh1_preparation_form):
            ...
            if label_text == "Date Received":
                ...
                wh1_preparation_form_entries.append(date_entry)
                ...
                wh1_preparation_form_entries.append(combobox)
                ...
                wh1_preparation_form_entries.append(status_combobox)

            else:
                entry = ttk.Entry(entry_frame, width=15)
                entry.grid(row=1, column=i, padx=10, pady=5)
                wh1_preparation_form_entries.append(entry)

        self.wh1_preparation_form_entries = wh1_preparation_form_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh1_preparation_form(wh1_preparation_form),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh1_preparation_form(wh1_preparation_form),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh1_preparation_form(wh1_preparation_form),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh1_preparation_form(wh1_preparation_form, clear_ui_only=True),
            width=10
        )
        ...
        entry_frame.grid_columnconfigure(len(column_names_wh1_preparation_form) - 1, weight=1)

    def add_row_wh1_preparation_form(self, table):
        try:
            self.connect_db()  # Ensure the connection is open

            # Retrieve values from entry fields for Table 1
            reference_no = self.wh1_preparation_form_entries[0].get()
            date = self.wh1_preparation_form_entries[1].get()
            material_code = self.wh1_preparation_form_entries[2].get()
            quantity_prepared = self.wh1_preparation_form_entries[3].get()
            quantity_return = self.wh1_preparation_form_entries[4].get()
            area_location = self.wh1_preparation_form_entries[5].get()
            status = self.wh1_preparation_form_entries[6].get()

            # Ensure inputs are not empty
            if not reference_no or not date or not material_code or not quantity_prepared or not quantity_return or not area_location or not status:
            ...
            messagebox.showinfo("Material Code Status Match",
            ...
            query = """INSERT INTO wh1_preparation_form (reference_no, date, material_code, quantity_prepared, quantity_return, area_location, status) 
                       VALUES (%s, %s, %s, %s, %s, %s, %s)"""
            values = (
            reference_no, date, material_code_id, quantity_prepared, quantity_return, f"Warehouse {area_location}",
            status)
            ...
            data_wh1_preparation_form = self.fetch_data_from_wh1_preparation_form()
            self.update_treeview(table, data_wh1_preparation_form,
    ...
    def update_row_wh1_preparation_form(self, table):
        try:
            ...
            self.cursor.execute("SELECT id FROM wh1_preparation_form WHERE reference_no = %s", (reference_no,))
            ...
            date = self.wh1_preparation_form_entries[1].get().strip()
            material_code = self.wh1_preparation_form_entries[2].get().strip()
            quantity_prepared = self.wh1_preparation_form_entries[3].get().strip()
            quantity_return = self.wh1_preparation_form_entries[4].get().strip()
            area_location = self.wh1_preparation_form_entries[5].get().strip()
            status = self.wh1_preparation_form_entries[6].get().strip()
            ...
            query = "UPDATE wh1_preparation_form SET "
            values = []
            ...
            if area_location:
                updated_values[5] = area_location
            if status:
    ...
    def delete_row_wh1_preparation_form(self, table):
        ...
        try:
            ...
            query = "SELECT id FROM wh1_preparation_form WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh1_preparation_form WHERE id = %s;"
    ...
    def clear_row_wh1_preparation_form(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh1_preparation_form SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                ...
                wpf.area_location,
                wpf.status 
            FROM wh1_preparation_form wpf
            JOIN material_codes mc ON wpf.material_code = mc.mid  -- Ensure 'mid' is the correct key
