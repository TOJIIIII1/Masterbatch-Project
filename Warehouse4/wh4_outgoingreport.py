import psycopg2
import ttkbootstrap as ttk  # Use ttk from ttkbootstrap
...
class Wh4OutgoingReport:
    ...
    def fetch_wh4_material_codes(self):
    ...
    def fetch_data_from_wh4_outgoing_report(self):
        """Fetch the latest data from Table 1 with date format MM/DD/YYYY."""
        try:
            self.connect_db()  # Ensure connection is open before executing the query
            query = """
                SELECT wh4_outgoing_report.reference_no, 
                       TO_CHAR(wh4_outgoing_report.date_outgoing, 'MM/DD/YYYY') AS date_outgoing, 
                       wh4_material_codes.material_code, 
                       wh4_outgoing_report.quantity, 
                       wh4_outgoing_report.area_location 
                FROM wh4_outgoing_report
                INNER JOIN wh4_material_codes
                    ON wh4_outgoing_report.material_code = wh4_material_codes.mid
                WHERE wh4_outgoing_report.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 4: Outgoing Report", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh4_outgoing_report, search_entry.get()))
        ...
        column_names_wh4_outgoing_report = [
            "Reference No.", "Date Outgoing", "Material Code", "Quantity", "Area Location"
        ]
        wh4_outgoing_report = ttk.Treeview(
            table_frame,
            columns=column_names_wh4_outgoing_report,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh4_outgoing_report.yview)

        for col_name in column_names_wh4_outgoing_report:
            wh4_outgoing_report.heading(col_name, text=col_name)
            wh4_outgoing_report.column(col_name, width=150, anchor="center")
        wh4_outgoing_report.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh4_outgoing_report = self.fetch_data_from_wh4_outgoing_report()
        self.update_treeview(wh4_outgoing_report, data_wh4_outgoing_report,
                             column_names_wh4_outgoing_report)
        ...
        wh4_outgoing_report_entries = []
        wh4_material_codes = self.fetch_wh4_material_codes()

        for i, label_text in enumerate(column_names_wh4_outgoing_report):
            ...
            if label_text == "Date Outgoing":  # Correct label for the date field
                ...
                wh4_outgoing_report_entries.append(date_entry)
                ...
                combobox = ttk.Combobox(entry_frame, values=wh4_material_codes, width=15, textvariable=combobox_var)
                combobox.grid(row=1, column=i, padx=10, pady=5)

                # Trace changes and enforce uppercase
                combobox_var.trace_add("write", to_uppercase)

                wh4_outgoing_report_entries.append(combobox)
            else:
                entry = ttk.Entry(entry_frame, width=15)
                entry.grid(row=1, column=i, padx=10, pady=5)
                wh4_outgoing_report_entries.append(entry)

        self.wh4_outgoing_report_entries = wh4_outgoing_report_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh4_outgoing_report(wh4_outgoing_report),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh4_outgoing_report(wh4_outgoing_report),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh4_outgoing_report(wh4_outgoing_report),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh4_outgoing_report(wh4_outgoing_report, clear_ui_only=True),
            width=10
        )
        ...
        entry_frame.grid_columnconfigure(len(column_names_wh4_outgoing_report) - 1, weight=1)

    # Other methods (add_row_wh4_outgoing_report, update_row_wh4_outgoing_report, etc.) remain the same.

    def add_row_wh4_outgoing_report(self, table):
        try:
            self.connect_db()  # Ensure the connection is open

            # Retrieve values from entry fields for Table 4
            reference_no = self.wh4_outgoing_report_entries[0].get()
            date_outgoing = self.wh4_outgoing_report_entries[1].get()
            material_code = self.wh4_outgoing_report_entries[2].get()
            quantity = self.wh4_outgoing_report_entries[3].get()
            ...
            # Get material_code_id from the wh4_material_codes table
            ...
            # Check the available total quantity in wh4_material_code_totals
            ...
            # Insert a new row into PostgreSQL (wh4_outgoing_report table)
            query = """INSERT INTO wh4_outgoing_report (reference_no, date_outgoing, material_code, quantity, area_location) 
                       VALUES (%s, %s, %s, %s, %s)"""
            values = (reference_no, date_outgoing, material_code_id, quantity, "Warehouse 4")
            self.cursor.execute(query, values)
            self.conn.commit()

            messagebox.showinfo("Success", "Row added successfully to Warehouse 4: Outgoing Report.")

            # After adding the row, refresh the Treeview to show the updated data
            data_wh4_outgoing_report = self.fetch_data_from_wh4_outgoing_report()
            self.update_treeview(table, data_wh4_outgoing_report,
                                 ["Reference No.", "Date Outgoing", "Material Code", "Quantity", "Area Location"])

        except Exception as e:
            messagebox.showerror("Error", f"Error while adding row to Warehouse 4: Outgoing Report: {e}")
            self.conn.rollback()  # Ensure rollback on error
        finally:
            self.close_connection()

    def update_row_wh4_outgoing_report(self, table):
        try:
            ...
            self.cursor.execute("SELECT id FROM wh4_outgoing_report WHERE reference_no = %s", (old_reference_no,))
            ...
            new_reference_no = self.wh4_outgoing_report_entries[0].get().strip()  # ✅ Allow updating Reference No.
            date_outgoing = self.wh4_outgoing_report_entries[1].get().strip()
            ...
            query = "UPDATE wh4_outgoing_report SET "
            values = []
    ...
    def delete_row_wh4_outgoing_report(self, table):
        ...
        try:
            ...
            query = "SELECT id FROM wh4_outgoing_report WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh4_outgoing_report WHERE id = %s;"
    ...
    def clear_row_wh4_outgoing_report(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh4_outgoing_report SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                wor.reference_no, 
                TO_CHAR(wor.date_outgoing, 'MM/DD/YYYY') AS date_outgoing, 
                mc.material_code, 
                wor.quantity, 
                wor.area_location 
            FROM wh4_outgoing_report wor
            JOIN wh4_material_codes mc ON wor.material_code = mc.mid  -- Updated JOIN condition
            WHERE wor.reference_no::TEXT ILIKE %s OR mc.material_code::TEXT ILIKE %s;
            """
