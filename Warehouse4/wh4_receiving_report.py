...
class Wh4ReceivingReport:
    ...
    def fetch_data_from_wh4_receiving_report(self):
        """Fetch the latest data from Table 1 where 'deleted' is FALSE."""
        try:
            query = """
                SELECT wh4_receiving_report.reference_no, 
                       TO_CHAR(wh4_receiving_report.date_received, 'MM/DD/YYYY') AS date_received, 
                       wh4_material_codes.material_code, 
                       wh4_receiving_report.quantity, 
                       wh4_receiving_report.area_location 
                FROM wh4_receiving_report
                INNER JOIN wh4_material_codes
                    ON wh4_receiving_report.material_code = wh4_material_codes.mid
                WHERE wh4_receiving_report.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 4: Receiving Report", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh4_receiving_report, search_entry.get()))
        ...
        column_names_wh4_receiving_report = [
            "Reference No.", "Date Received", "Material Code", "Quantity", "Area Location"
        ]
        wh4_receiving_report = ttk.Treeview(
            table_frame,
            columns=column_names_wh4_receiving_report,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh4_receiving_report.yview)

        for col_name in column_names_wh4_receiving_report:
            wh4_receiving_report.heading(col_name, text=col_name)
            wh4_receiving_report.column(col_name, width=150, anchor="center")
        wh4_receiving_report.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh4_receiving_report = self.fetch_data_from_wh4_receiving_report()
        self.update_treeview(wh4_receiving_report, data_wh4_receiving_report,
                             column_names_wh4_receiving_report)
        ...
        wh4_receiving_report_entries = []
        wh4_material_codes = self.fetch_wh4_material_codes()

        for i, label_text in enumerate(column_names_wh4_receiving_report):
            ...
            if label_text == "Date Received":
                ...
                wh4_receiving_report_entries.append(date_entry)
                ...
                wh4_receiving_report_entries.append(combobox)
            else:
                entry = ttk.Entry(entry_frame, width=15)
                entry.grid(row=1, column=i, padx=10, pady=5)
                wh4_receiving_report_entries.append(entry)

        self.wh4_receiving_report_entries = wh4_receiving_report_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh4_receiving_report(wh4_receiving_report),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh4_receiving_report(wh4_receiving_report),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh4_receiving_report(wh4_receiving_report),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh4_receiving_report(wh4_receiving_report, clear_ui_only=True),
            width=10
        )
        ...
        entry_frame.grid_columnconfigure(len(column_names_wh4_receiving_report) - 1, weight=1)

    def add_row_wh4_receiving_report(self, table):
        try:
            self.connect_db()  # Ensure the connection is open

            # Retrieve values from entry fields for Table 1
            reference_no = self.wh4_receiving_report_entries[0].get()
            date_received = self.wh4_receiving_report_entries[1].get()
            material_code = self.wh4_receiving_report_entries[2].get()
            quantity = self.wh4_receiving_report_entries[3].get()
            ...
            # Insert a new row into PostgreSQL (wh4_receiving_report table)
            query = """INSERT INTO wh4_receiving_report (reference_no, date_received, material_code, quantity, area_location) 
                       VALUES (%s, %s, %s, %s, %s)"""
            values = (reference_no, date_received, material_code_id, quantity, "Warehouse 4")
            self.cursor.execute(query, values)
            self.conn.commit()

            messagebox.showinfo("Success", "Row added successfully to Warehouse 4: Receiving Report.")

            # After adding the row, refresh the Treeview to show the updated data
            data_wh4_receiving_report = self.fetch_data_from_wh4_receiving_report()
            self.update_treeview(table, data_wh4_receiving_report,
                                 ["Reference No.", "Date Received", "Material Code", "Quantity", "Area Location"])

        except Exception as e:
            messagebox.showerror("Error", f"Error while adding row to Warehouse 4: Receiving Report: {e}")
            self.conn.rollback()  # Rollback the transaction if there's an error
        finally:
            self.close_connection()

    def update_row_wh4_receiving_report(self, table):
        try:
            ...
            self.cursor.execute("SELECT id FROM wh4_receiving_report WHERE reference_no = %s", (old_reference_no,))
            ...
            new_reference_no = self.wh4_receiving_report_entries[0].get().strip()  # ✅ Add Reference No. field
            date_received = self.wh4_receiving_report_entries[1].get().strip()
            material_code = self.wh4_receiving_report_entries[2].get().strip()
            quantity = self.wh4_receiving_report_entries[3].get().strip()
            ...
            query = "UPDATE wh4_receiving_report SET "
            values = []
    ...
    def delete_row_wh4_receiving_report(self, table):
        ...
        try:
            ...
            query = "SELECT id FROM wh4_receiving_report WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh4_receiving_report WHERE id = %s;"
    ...
    def clear_row_wh4_receiving_report(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh4_receiving_report SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                ...
                wrr.quantity, 
                wrr.area_location 
            FROM wh4_receiving_report wrr
            JOIN wh4_material_codes mc ON wrr.material_code = mc.mid  -- Ensure 'mid' is the correct key
