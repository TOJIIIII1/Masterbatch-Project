...
class Wh4TransferForm:
    ...
    def fetch_data_from_wh4_transfer_form(self):
        """Fetch the latest data from Table 1 with date format MM/DD/YYYY."""
        try:
            self.connect_db()  # Ensure connection is open before executing the query
            query = """
                SELECT wh4_transfer_form.reference_no, 
                       TO_CHAR(wh4_transfer_form.date, 'MM/DD/YYYY') AS date, 
                       material_codes.material_code, 
                       wh4_transfer_form.quantity, 
                       wh4_transfer_form.area_to,
                       wh4_transfer_form.status,
                       'Change Status' AS new_status
                FROM wh4_transfer_form
                INNER JOIN material_codes
                    ON wh4_transfer_form.material_code = material_codes.mid
                WHERE wh4_transfer_form.deleted = FALSE;
            """
            self.cursor.execute(query)
    ...
    def display(self, parent_frame):
        ...
        label = ttk.Label(parent_frame, text="Warehouse 4: Transfer Form", style="Custom.TLabel")
        ...
        search_entry.bind("<KeyRelease>",
                          lambda event: self.search_table(wh4_transfer_form, search_entry.get()))
        ...
        column_names_wh4_transfer_form = [
            "Reference No.", "Date Received", "Material Code", "Quantity", "Area To", "Status"
        ]

        wh4_transfer_form = ttk.Treeview(
            table_frame,
            columns=column_names_wh4_transfer_form,
            show="headings",
            yscrollcommand=scrollbar.set,
            height=15
        )
        scrollbar.config(command=wh4_transfer_form.yview)

        for col_name in column_names_wh4_transfer_form:
            wh4_transfer_form.heading(col_name, text=col_name)
            wh4_transfer_form.column(col_name, width=150, anchor="center")
        wh4_transfer_form.pack(fill="both", expand=True)

        # Fetch data and populate the table
        data_wh4_transfer_form = self.fetch_data_from_wh4_transfer_form()
        self.update_treeview(wh4_transfer_form, data_wh4_transfer_form,
                             column_names_wh4_transfer_form)
        ...
        wh4_transfer_form_entries = []
        material_codes = self.fetch_wh4_material_codes()

        for i, label_text in enumerate(column_names_wh4_transfer_form):
            ...
            if label_text == "Date Received":
                ...
                wh4_transfer_form_entries.append(date_entry)
                ...
                wh4_transfer_form_entries.append(combobox)
                ...
                wh4_transfer_form_entries.append(area_combobox)
        ...
        additional_combobox.grid(row=1, column=len(column_names_wh4_transfer_form), padx=10, pady=5)

        # Store in a class attribute if needed
        self.additional_combobox = additional_combobox

        self.wh4_transfer_form_entries = wh4_transfer_form_entries
        ...
        add_button = ttk.Button(
            button_frame,
            text="Add",
            command=lambda: self.add_row_wh4_transfer_form(wh4_transfer_form),
            width=10
        )
        ...
        update_button = ttk.Button(
            button_frame,
            text="Update",
            command=lambda: self.update_row_wh4_transfer_form(wh4_transfer_form),
            width=10
        )
        ...
        delete_button = ttk.Button(
            button_frame,
            text="Delete",
            command=lambda: self.delete_row_wh4_transfer_form(wh4_transfer_form),
            width=10
        )
        ...
        clear_button = ttk.Button(
            button_frame,
            text="Clear",
            command=lambda: self.clear_row_wh4_transfer_form(wh4_transfer_form, clear_ui_only=True),
            width=10
        )
        ...
        entry_frame.grid_columnconfigure(len(column_names_wh4_transfer_form) - 1, weight=1)

    # Other methods (add_row_wh4_transfer_form, update_row_wh4_transfer_form, etc.) remain the same.

    def add_row_wh4_transfer_form(self, table):
        try:
            self.connect_db()  # Ensure the connection is open

            # Retrieve values from entry fields
            reference_no = self.wh4_transfer_form_entries[0].get()
            date = self.wh4_transfer_form_entries[1].get()
            material_code = self.wh4_transfer_form_entries[2].get()
            ...
            get_material_codes_with_status = """
                SELECT material_code_name FROM wh4_material_code_totals WHERE status = %s
            """
            ...
            get_total_quantity = """
                SELECT total_quantity FROM wh4_material_code_totals WHERE material_code_name = %s AND status = %s
            """
            ...
            # Step 5: Insert into the wh4_transfer_form
            query = """
            ...
            if area_to == "1":
                ...
                """
                self.cursor.execute(insert_wh4_query,
                                    (reference_no, date, material_code_id, quantity, "From Warehouse 2", new_status))
                self.conn.commit()
    ...
    def update_row_wh4_transfer_form(self, table):
        try:
            self.connect_db()  # Ensure the connection is open
            ...
            # Retrieve values from entry fields
            date = self.wh4_transfer_form_entries[1].get().strip()
            material_code = self.wh4_transfer_form_entries[2].get().strip()
            quantity = self.wh4_transfer_form_entries[3].get().strip()
            area_to = self.wh4_transfer_form_entries[4].get().strip()
            ...
            query = "UPDATE wh4_transfer_form SET "
            values = []
    ...
    def delete_row_wh4_transfer_form(self, table):
        ...
        try:
            ...
            # Fetch the unique 'id' from wh4_receiving_report
            query = "SELECT id FROM wh4_transfer_form WHERE reference_no = %s AND material_code = %s LIMIT 1;"
            ...
            query = "DELETE FROM wh4_transfer_form WHERE id = %s;"
    ...
    def search_table(self, table, search_text):
        ...
        try:
            ...
            SELECT 
                ...
                wtf.quantity, 
                wtf.area_to
            FROM wh4_transfer_form wtf
            JOIN wh4_material_codes mc ON wtf.material_code = mc.mid  -- Ensure 'mid' is the correct key
    ...
    def clear_row_wh4_transfer_form(self, table, clear_ui_only=True):
        """
        ...
        try:
            ...
            if clear_ui_only:
                ...
                query = "UPDATE wh4_transfer_form SET deleted = TRUE WHERE deleted = FALSE;"
                self.cursor.execute(query)
                self.conn.commit()
